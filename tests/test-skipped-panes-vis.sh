#!/usr/bin/env bash
# T5 회귀: SKIPPED_PANES 누적되면 team-up 끝에서 stderr 출력.
# team-up.sh 실제 통합 테스트 어려움 — 끝부분 가시화 로직만 wrapper 로 검증.
# 추가로 team-up.sh 본체에 그 echo 라인이 실재하는지 정적 grep 으로 검증.
...
# 임시 wrapper — team-up.sh 끝부분의 SKIPPED_PANES 가시화 로직만 추출 모방.
TMP_WRAP="$(mktemp)"
TMP_WRAP2="$(mktemp)"
...
cat > "$TMP_WRAP" <<'EOF'
#!/usr/bin/env bash
# team-up.sh 끝부분의 SKIPPED_PANES 가시화 로직 모방.
...
# T5.4 분해: team-up.sh 본체의 가시화 코드를 wording 무감각·env 계약 민감으로 검증.
# 단일 wording grep 은 한 글자 변경에도 깨졌었음 → 다중 grep 으로 strict 결합 해소.

# T5.4a: warning marker 존재 (약한 wording 결합 — "주의:" 또는 "warning" 류 허용).
if grep -qE '주의:|[Ww]arning' "$HARNESS_ROOT/bin/team-up.sh"; then
  assert_eq "marker-present" "marker-present" "team-up.sh 에 warning marker 존재 (주의/warning)"
else
  assert_eq "marker-present" "missing" "team-up.sh 에 warning marker 존재 (주의/warning)"
fi

# T5.4b: SHELL_READY_TIMEOUT env 이름 멘션 (강한 계약 — env 이름은 README·코드 동시 계약).
if grep -q "SHELL_READY_TIMEOUT" "$HARNESS_ROOT/bin/team-up.sh"; then
  assert_eq "shell-ready-timeout-mention" "shell-ready-timeout-mention" "team-up.sh 가 SHELL_READY_TIMEOUT env 멘션"
else
  assert_eq "shell-ready-timeout-mention" "missing" "team-up.sh 가 SHELL_READY_TIMEOUT env 멘션"
fi

# T5.4c: BOOT_REPL_CHECK_DELAY env 이름 멘션 (위와 동일 강한 계약).
if grep -q "BOOT_REPL_CHECK_DELAY" "$HARNESS_ROOT/bin/team-up.sh"; then
  assert_eq "boot-repl-check-delay-mention" "boot-repl-check-delay-mention" "team-up.sh 가 BOOT_REPL_CHECK_DELAY env 멘션"
else
  assert_eq "boot-repl-check-delay-mention" "missing" "team-up.sh 가 BOOT_REPL_CHECK_DELAY env 멘션"
fi
