#!/usr/bin/env bash
# tests/test-profile-reviewers.sh — feature-team.sh REVIEWERS 배열 정합 검증
...
# Layer 1: REVIEWERS 배열 4개 토큰
grep -q 'spec-rev:reviewer-spec' "$ROOT/profiles/feature-team.sh"
assert_success "$?" "L1 spec-rev 토큰"
grep -q 'quality-rev:reviewer-quality' "$ROOT/profiles/feature-team.sh"
assert_success "$?" "L1 quality-rev 토큰"
grep -q 'arch-rev:reviewer-arch' "$ROOT/profiles/feature-team.sh"
assert_success "$?" "L1 arch-rev 토큰"
grep -q 'review-mgr:review-manager' "$ROOT/profiles/feature-team.sh"
assert_success "$?" "L1 review-mgr 토큰 (Task 6 추가)"
