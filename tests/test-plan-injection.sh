...
# rc 먼저 검증 — "합본 미생성" 과 "agenphony-up 자체 실패" 를 구분(진단성, test-boot-tokens 패턴).
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan "$PLAN_A" default >/tmp/_t-plan-out.log 2>/tmp/_t-plan-err.log
rc=$?
assert_eq "0" "$rc" "--plan 단일: agenphony-up 성공 (rc=$rc, err=$(head -3 /tmp/_t-plan-err.log 2>/dev/null))"
sleep 0.8
[ -f "$PLAN_BOOT" ]
...
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan "$PLAN_A" --plan "$PLAN_B" default >/dev/null 2>&1
sleep 0.8
...
bash "$ROOT/bin/agenphony-up.sh" default --project "$PROJ" --plan "$PLAN_A" >/dev/null 2>&1
sleep 0.8
[ -f "$PLAN_BOOT" ]
...
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan /nonexistent/xyz.md default 2>/dev/null
assert_fail "$?" "--plan 비존재 파일 → 비-0 종료"

# 케이스 6: --plan 값 누락 → 오류 (비-0).
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan 2>/dev/null
assert_fail "$?" "--plan 값 누락 → 비-0 종료"

# 케이스 7: agenphony-down 이 --plan 을 받아도 오류 없이 무시 (인자 대칭).
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan "$PLAN_A" default >/dev/null 2>&1
sleep 0.8
bash "$ROOT/bin/agenphony-down.sh" --project "$PROJ" --plan "$PLAN_A" >/dev/null 2>&1
assert_success "$?" "agenphony-down --plan: 미지옵션 오류 없이 무시 (대칭)"

# 케이스 8: agenphony-down --plan 값 누락(agenphony-up 인자 실수 복붙) → set -e 사망 없이 정상 정리.
bash "$ROOT/bin/agenphony-up.sh" --project "$PROJ" --plan "$PLAN_A" default >/dev/null 2>&1
sleep 0.8
bash "$ROOT/bin/agenphony-down.sh" --project "$PROJ" --plan >/dev/null 2>&1
assert_success "$?" "agenphony-down --plan 값누락: set -e 사망 없이 무시 (대칭)"
