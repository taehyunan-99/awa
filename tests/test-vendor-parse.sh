...
out="$( set +u; HARNESS_ROOT="$ROOT"; HARNESS_VENDOR="codex"
  . "$ROOT/bin/lib.sh"
  eval "$(extract_parse_entry)"
  parse_entry "dev:dev"          # ENTRY_MODEL="" 기대
  if [ -z "$ENTRY_MODEL" ]; then
    . "$ROOT/bin/vendors/${ENTRY_VENDOR:-${HARNESS_VENDOR:-claude}}.sh"
...
out="$( set +u; HARNESS_ROOT="$ROOT"; HARNESS_VENDOR="claude"
  . "$ROOT/bin/lib.sh"
  eval "$(extract_parse_entry)"
  parse_entry "dev:dev"
  if [ -z "$ENTRY_MODEL" ]; then
