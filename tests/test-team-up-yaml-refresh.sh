#!/usr/bin/env bash
# team-up 의 lead-auto-allow.yaml marker 게이트 + 백업 갱신 검증 (8차).
...
run_teamup() { HARNESS_PROJECT="$TMP" AGENT_CMD=cat bash "$ROOT/bin/team-up.sh" feature-team >/dev/null 2>&1; tmux kill-session -t "$SESSION" 2>/dev/null || true; }

echo "[YR1] 최초설치: yaml 없음 → 복사 + marker 생성"
run_teamup
[ -f "$YAML" ];   assert_success "$?" "YR1 yaml 생성됨"
[ -f "$MARKER" ]; assert_success "$?" "YR1 marker 생성됨"
cmp -s "$ROOT/config/lead-auto-allow.yaml" "$YAML"; assert_success "$?" "YR1 하네스와 동일"

echo "[YR2] marker있음+같음: 재실행 → 백업 안 만듦(스킵)"
run_teamup
bak_count="$(ls "$TMP/config/"*.bak 2>/dev/null | wc -l | tr -d ' ')"
...
cmp -s "$ROOT/config/lead-auto-allow.yaml" "$YAML"; assert_success "$?" "YR3 하네스 최신으로 교체됨"
grep -q OLDVERSION "$TMP/config/"*.bak; assert_success "$?" "YR3 구버전이 .bak 에 보존"

echo "[YR4] marker없음+있음: 사용자 작성 → 보존(불변) + 경고"
rm -f "$TMP/config/"*.bak "$MARKER"                          # marker 제거 = 사용자 작성 취급
printf 'read-only:\n  - "Bash(USERCUSTOM:*)"\n' > "$YAML"
warn="$(HARNESS_PROJECT="$TMP" AGENT_CMD=cat bash "$ROOT/bin/team-up.sh" feature-team 2>&1 >/dev/null)"
