...
trap '
  ...
  tmux kill-session -t awa-mf-test-$$ 2>/dev/null
' EXIT

echo "[M1] mode=launch 단일 single + current cwd: 발진 명령 출력"
TMP="$(mktemp -d)"; ( cd "$TMP" && git init -q )
out="$(bash "$HARNESS_BIN/awa-main.sh" launch \
       --project "$TMP" --mode-launch single --preset default 2>&1)"
assert_contains "$out" "awa-up.sh" "M1a 발진 명령 출력"
...
out="$(bash "$HARNESS_BIN/awa-main.sh" attach --session awa-foo 2>&1)"
assert_contains "$out" "tmux attach -t awa-foo" "M2 attach 명령"

echo "[M3] mode=resume: live 세션 목록 출력 (TSV 형태)"
S="awa-mf-test-$$"
TMP2="$(mktemp -d)"
tmux new-session -d -s "$S"
tmux set-option -t "$S" @awa-project "$TMP2" 2>/dev/null
out="$(bash "$HARNESS_BIN/awa-main.sh" resume 2>&1)"
...
# 외부 awa-* / _SYMPHONY 가 없을 때만 검증
live_check=$(tmux ls -F '#{session_name}' 2>/dev/null | grep -cE '^(awa-|_SYMPHONY$)' 2>/dev/null) || live_check=0
if [ "$live_check" = 0 ]; then
  ...
  echo "  (SKIP M3b — 외부 awa-*/_SYMPHONY 존재)"
fi
...
if bash "$HARNESS_BIN/awa-main.sh" launch --mode-launch single --preset default 2>/dev/null; then
...
out="$(bash "$HARNESS_BIN/awa-main.sh" resolve-path --input "m8alias" 2>&1)"
assert_eq "/tmp/agpn-m8-real" "$out" "M8a alias → path"
# path 직접 통과
out="$(bash "$HARNESS_BIN/awa-main.sh" resolve-path --input "/tmp/agpn-m8-real" 2>&1)"
assert_eq "/tmp/agpn-m8-real" "$out" "M8b path 직접"
# 실패 케이스
if bash "$HARNESS_BIN/awa-main.sh" resolve-path --input "/no-such-path-xyz" 2>/dev/null; then
...
if bash "$HARNESS_BIN/awa-main.sh" launch --project "$TMP9" --mode-launch single --preset 'foo; rm -rf' 2>/dev/null; then
