...
out="$(bash "$ROOT/bin/awa-main.sh" launch \
...
out="$(bash "$ROOT/bin/awa-main.sh" attach --session awa-foo 2>&1)"
...
out="$(bash "$ROOT/bin/awa-main.sh" resume 2>&1)"
...
if bash "$ROOT/bin/awa-main.sh" launch --mode-launch single --preset default 2>/dev/null; then
...
out="$(bash "$ROOT/bin/awa-main.sh" resolve-path --input "m8alias" 2>&1)"
assert_eq "/tmp/agpn-m8-real" "$out" "M8a alias → path"
# path 직접 통과
out="$(bash "$ROOT/bin/awa-main.sh" resolve-path --input "/tmp/agpn-m8-real" 2>&1)"
assert_eq "/tmp/agpn-m8-real" "$out" "M8b path 직접"
# 실패 케이스
if bash "$ROOT/bin/awa-main.sh" resolve-path --input "/no-such-path-xyz" 2>/dev/null; then
...
if bash "$ROOT/bin/awa-main.sh" launch --project "$TMP9" --mode-launch single --preset 'foo; rm -rf' 2>/dev/null; then
