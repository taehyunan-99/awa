...
if grep -n "lead-ask\|pm pane 알림 채널" "$ROOT/bin/awa-up.sh" >/dev/null 2>&1; then
  assert_eq "0건" "발견됨" "awa-up.sh 에 @lead-ask 잔존: $(grep -n 'lead-ask\|pm pane 알림 채널' "$ROOT/bin/awa-up.sh")"
else
