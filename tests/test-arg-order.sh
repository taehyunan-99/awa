...
bash "$ROOT/bin/team-down.sh" --project "$PROJ_A" >/dev/null 2>&1
...
bash "$ROOT/bin/team-down.sh" default --project "$PROJ_B" >/dev/null 2>&1
tmux has-session -t "$SES_B" 2>/dev/null
assert_fail "$?" "--project 뒤: team-down 으로 세션 종료 (결함 C 양방향)"

# 케이스 3: --project 인자 누락 → 오류
bash "$ROOT/bin/team-up.sh" --project 2>/dev/null
