...
cleanup() {
  ...
  for p in "$PROJ_A" "$PROJ_B"; do
    s="$(HARNESS_PROJECT="$p" bash -c "source $HARNESS_BIN/lib.sh; resolve_session" 2>/dev/null || true)"
...
SES_A="$(HARNESS_PROJECT="$PROJ_A" bash -c "source $HARNESS_BIN/lib.sh; resolve_session")"
SES_B="$(HARNESS_PROJECT="$PROJ_B" bash -c "source $HARNESS_BIN/lib.sh; resolve_session")"

# 케이스 1: --project 가 프로파일명 앞 (기존에도 됨)
bash "$HARNESS_BIN/awa-up.sh" --project "$PROJ_A" default >/dev/null 2>&1
sleep 0.8  # awa-up 의 .agent-harness 마커 생성 여유 (저사양/CI 간헐 실패 방어)
...
bash "$HARNESS_BIN/awa-down.sh" --project "$PROJ_A" >/dev/null 2>&1

# 케이스 2: --project 가 프로파일명 뒤 (결함 C — 예전엔 무시됨)
bash "$HARNESS_BIN/awa-up.sh" default --project "$PROJ_B" >/dev/null 2>&1
sleep 0.8  # awa-up 의 .agent-harness 마커 생성 여유 (저사양/CI 간헐 실패 방어)
...
bash "$HARNESS_BIN/awa-down.sh" default --project "$PROJ_B" >/dev/null 2>&1
tmux has-session -t "$SES_B" 2>/dev/null
assert_fail "$?" "--project 뒤: awa-down 으로 세션 종료 (결함 C 양방향)"

# 케이스 3: --project 인자 누락 → 오류
bash "$HARNESS_BIN/awa-up.sh" --project 2>/dev/null
assert_fail "$?" "--project 값 누락 → 비-0 종료"

# 케이스 4: --project 경로 없음 → 오류
bash "$HARNESS_BIN/awa-up.sh" --project /nonexistent/path/xyz 2>/dev/null
assert_fail "$?" "--project 경로 없음 → 비-0 종료"

# 케이스 5: 등호형 --project=/path (분리형과 동등 동작)
PROJ_C="$(mktemp -d)"; ( cd "$PROJ_C" && git init -q )
SES_C="$(HARNESS_PROJECT="$PROJ_C" bash -c "source $HARNESS_BIN/lib.sh; resolve_session")"
bash "$HARNESS_BIN/awa-up.sh" "default" "--project=$PROJ_C" >/dev/null 2>&1
sleep 0.8
...
bash "$HARNESS_BIN/awa-down.sh" "--project=$PROJ_C" >/dev/null 2>&1
tmux kill-session -t "$SES_C" 2>/dev/null || true
rm -rf "$PROJ_C"

# 케이스 6: 알 수 없는 옵션(-*) → 즉시 거부 (오타 침묵 무시 방지)
bash "$HARNESS_BIN/awa-up.sh" --bogus-flag 2>/dev/null
