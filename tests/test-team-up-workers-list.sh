...
echo "[U7] ★ awa-up 가 PROJECT_ROOT/.agent-harness/config/orch-auto-allow.yaml 설치 (실전 부트 경로)"
# 실전 결함: orch_auto_allow_lookup 은 PROJECT_ROOT/.agent-harness/config/orch-auto-allow.yaml 을
#   읽는데(Task 2 이전), awa-up 이 설치 안 하면 파일 부재 → lookup 영구 rc=1 →
#   orch-auto-allow 전체 무동작. 부트 후 yaml 이 PROJECT_ROOT 에 있어야 함.
[ -f "$TMP/.agent-harness/config/orch-auto-allow.yaml" ]; assert_success "$?" "U7 yaml PROJECT_ROOT 설치됨"
# 내용이 하네스 원본과 동일 (복사 검증)
if [ -f "$TMP/.agent-harness/config/orch-auto-allow.yaml" ]; then
  assert_contains "$(cat "$TMP/.agent-harness/config/orch-auto-allow.yaml")" "safe-test" "U7 yaml 내용 (safe-test 카테고리)"
fi
