...
# A·B 동시 가동
HARNESS_PROJECT="$TMP1" AGENT_CMD=cat bash "$ROOT/bin/agenphony-up.sh" default >/dev/null 2>&1
sleep 0.3
HARNESS_PROJECT="$TMP2" AGENT_CMD=cat bash "$ROOT/bin/agenphony-up.sh" default >/dev/null 2>&1
sleep 0.3

# T16.1 — 두 SESSION 공존
tmux has-session -t agenphony-projectA 2>/dev/null; assert_success "$?" "agenphony-projectA 살아있음"
tmux has-session -t agenphony-projectB 2>/dev/null; assert_success "$?" "agenphony-projectB 살아있음"
...
# T16.5 — A agenphony-down 이 B 영향 없음
HARNESS_PROJECT="$TMP1" bash "$ROOT/bin/agenphony-down.sh" >/dev/null 2>&1
sleep 0.3
! tmux has-session -t agenphony-projectA 2>/dev/null; assert_success "$?" "A 세션 종료됨"
tmux has-session -t agenphony-projectB 2>/dev/null; assert_success "$?" "B 세션 여전히 살아있음"
[ -d "$TMP2/.agent-harness" ]; assert_success "$?" "B .agent-harness/ 여전히"

# T16.6 — B 도 정리
HARNESS_PROJECT="$TMP2" bash "$ROOT/bin/agenphony-down.sh" >/dev/null 2>&1
sleep 0.3
! tmux has-session -t agenphony-projectB 2>/dev/null; assert_success "$?" "B 세션 종료됨"
