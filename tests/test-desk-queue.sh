...
# lead 페인에 @desk: <지시> 를 대신 전달한다. lead 페인은 cat(AGENT_CMD)이라 입력이 화면에
...
# 2) watcher 가 큐를 소비해 lead 페인에 @desk: 전달 → 화면에 마커 출현까지 대기(최대 ~8s).
got=1
for _i in $(seq 1 80); do
  if tmux capture-pane -t "$LEAD_PANE" -p 2>/dev/null | grep -q "@desk: $MARK"; then
    got=0; break
  fi
  sleep 0.1
done
assert_eq "0" "$got" "watcher 가 desk-queue 소비→lead 에 @desk: 전달(화면 도착)"
