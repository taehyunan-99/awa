#!/usr/bin/env bash
# config/orch-auto-allow.yaml 의 카테고리 매칭 (실제 yaml + matrix-lookup).
set -uo pipefail
cd "$(dirname "$0")"
source ./assert.sh
...
[ -f "$HARNESS_CONFIG/orch-auto-allow.yaml" ]; assert_success "$?" "yaml 존재"
...
out="$(orch_auto_allow_lookup Bash '{"command":"tmux wait-for -S done-awa-x-dev-T1"}')"
assert_eq "harness-infra:Bash(tmux wait-for:*)" "$out" "Y12 tmux wait-for → harness-infra"
out="$(orch_auto_allow_lookup Bash '{"command":"printf x >> .agent-harness/events.log"}')"
...
orch_auto_allow_lookup Bash '{"command":"git checkout -- todo.sh"}' >/dev/null
assert_fail "$?" "Y11 git checkout 은 회색 (데이터손실 인지)"
orch_auto_allow_lookup Bash '{"command":"git switch main"}' >/dev/null
...
out="$(orch_auto_allow_lookup Bash '{"command":"bash ./test.sh"}')"
assert_eq "self-verify:Bash(bash :*)" "$out" "Y11b bash ./X → self-verify"
out="$(orch_auto_allow_lookup Bash '{"command":". ./src/range.sh && range_sum 1 5"}')"
assert_eq "self-verify:Bash(. :*)" "$out" "Y11b . ./X → self-verify"
out="$(orch_auto_allow_lookup Bash '{"command":"source ./tests/t.sh"}')"
assert_eq "self-verify:Bash(source :*)" "$out" "Y11b source ./X → self-verify"
# ★ self-verify 는 orch_auto_allow_lookup 단독으론 시스템경로도 매칭(auto)하지만, classify 종단에선
...
out="$(orch_auto_allow_lookup Bash '{"command":"bash /etc/passwd"}')"
...
for c in "node index.js" "node ./server/app.js" "python app.py" "npx vite"; do
  orch_auto_allow_lookup Bash "{\"command\":\"$c\"}" >/dev/null
