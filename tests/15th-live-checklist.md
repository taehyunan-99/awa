...
- [ ] 발진 명령 출력 확인
- [ ] `!` 로 실행 → `awa-agpn15-a` 세션 발진
- [ ] `tmux attach -t awa-agpn15-a` 로 attach, LEAD/PM/workers/review 확인

## 2. bookmarks 자동 등록
- [ ] `! bash bin/awa-bookmarks.sh list` → `/tmp/agpn15-a` 항목 보임
- [ ] preset=default, last_used 타임스탬프 정상

## 3. 두번째 프로젝트 + Multi-view
- [ ] `! cd /tmp/agpn15-b` 후 `/agpn` 호출
- [ ] Step 0 Resume? → no
- [ ] Step 2: live_others=1 (awa-agpn15-a) → Multi-view 옵션 표시
- [ ] Multi-view 선택
- [ ] 발진 명령 출력
- [ ] `!` 로 실행 → awa-agpn15-b 발진
...
## 4. Symphony 액션
- [ ] `/agpn symphony detach awa-agpn15-a-team`
- [ ] _SYMPHONY 에서 a 제거, awa-agpn15-a 에 team 복원 확인
- [ ] _SYMPHONY 에 1개만 남음 → auto detach + kill 메시지
- [ ] `tmux ls` 로 _SYMPHONY 없음 확인

## 5. Resume + Bookmarks alias
- [ ] `! bash bin/awa-bookmarks.sh set-alias` → /tmp/agpn15-a 에 alias "a" 부여
...
## 정리
- [ ] `! bash bin/awa-down-menu.sh` (남은 세션 정리)
