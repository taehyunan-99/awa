...
- **task 명세 (단일 라인 압축)**: 각 task(`.agent-harness/tasks/<id>.md`)에 **objective**(무엇을) · **scope**(allowed_paths/forbidden_paths) · **output**(산출 경로) · **입력경로**(이전 산출물 연계 시) 명시. 카탈로그에서 적임 워커 선택 후 `{{HARNESS_ROOT}}/bin/dispatch.sh <worker> <id>`.
- **team-sizing**: 단순=워커 1·task 1 / 중간=워커 2~3 / 복잡=병렬 다수. 고정 풀 안에서만(신규 생성 금지). 1워커로 될 일에 여럿 쓰지 마라.
...
- **lead BLOCKED**: 막히면(모호·순환·권한·충돌) 추측 진행 마라 — 멈추고 사용자 push. 포맷 고정: **obstacle**(무엇이 막혔나) · **tried**(시도한 것) · **need**(필요 결정 1개).
...
- **good enough 면 종료**: 목표 충족 시 추가 위임·과보강 마라. (근거: 과배정은 비용 낭비·결과 noise.)
