...
## 후속 검증 — 후보 D(AB 하이브리드) 시도 (2026-05-26)

A 가 "후보 3개 중 최선" 이지만 "절대적 최선" 인지 의심 → A 골격(신호 6절) + B 강점 흡수한 D 제작·실험.

**D 의 A 대비 변경 4점**:
1. ⓑ task 명세 단일 라인 압축(B 식)
2. ⓑ team-sizing 가이드 추가(난이도→워커 수, 과배정 방지)
3. ⓔ BLOCKED 포맷 고정(`obstacle·tried·need` 라벨링)
4. ⓕ "good enough 면 종료" 절 추가(B 식)

42줄(A 40줄 +2).

### D plan-complex 실험 결과

| 항목 | A | D |
|---|---|---|
| 확정 plan 자동 착수 | ✅ | ❌ (boot 입력 프롬프트 "대기하라" 에 졌음 — 사용자 깨우기 필요) |
| 분해 품질 | T1·T2·T3 3 task | 동일(`deps:` 명시는 미세 우위) |
| `@done:` 종합 | ✅ verdict·증거 기록 | ✅ 동일 + reviewer false-positive 정확 판별 |
| **T1→T2 자동 순차 dispatch** | ✅ | ❌ **결정적 결함** — "ⓕ에 따라 T2 dispatch 는 @pm 지시 대기" 로 정지 |

### D 결정적 결함 분석

D 는 lead 화면에 명시 출력: `"단계 자동 전이 금지(ⓕ)에 따라 T2 dispatch 는 @pm 지시 대기. idle."`

ⓕ 의 `단계 자동 전이 금지 ... 단 확정 plan 착수는 예외` 가 "**착수**" 만 예외고 "**plan 안 순차 진행**" 은 예외에 안 들어간다고 해석. 거기에 D 가 추가한 ⓕ "good enough 면 종료" 가 "한 task 끝났으면 추가 위임 마라" 신호로 작용, 정지 결정을 강화.

**A 와의 차이**: A 는 ⓕ "good enough" 가 없어 plan 안 순차 진행이 자연스러웠음. 절을 늘리면 트리거가 흐려진다는 가설 추가 입증.

### 부수 발견 — boot 프롬프트가 자동 착수의 결정 변수

D 는 boot 직후 자동 분해 안 함 → 출력: `"사용자가 '준비되면 다음 지시를 대기하라' 고 명시했으므로 자동 분해·배정을 보류"`. 

`bin/agenphony-up.sh:496` 의 lead 부트 입력 프롬프트 `"...준비되면 다음 지시를 대기하라"` 가 lead.md ⓐ 자동 착수 트리거와 정면 충돌. A 가 자동 착수한 건 ⓐ 가 이긴 1회 운, D 는 진 1회. **lead.md 본문을 아무리 손봐도 boot 입력 프롬프트 문구가 결정 변수**. 별도 보강 필요(Task 15 안건).

### D vs A 결론 — A 유지 확정

D 채택 ❌. plan-complex 1셀 만으로 결정적 결함 확정(자동 순차 dispatch 실패). 추가 셀(stress-ambiguous 등) 진행 가치 약함.

**A 의 견고함 재확인**: 미니멀(C)·풍부함(B)·하이브리드(D) 모두 시도했지만 A 가 우위. "절을 늘리면 트리거가 흐려진다" 원칙 입증.

**남은 미해결 의심**: A 의 자동 착수도 boot 프롬프트와 충돌하는 1회 운이었다는 가능성. **Task 15 (별도)**: boot 프롬프트 분기(`--plan` 주입 시 "확정 plan 진행하라" 로 변경) 로 구조적 해결.
