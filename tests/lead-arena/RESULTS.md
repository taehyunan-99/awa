...
`bin/awa-up.sh:496` 의 lead 부트 입력 프롬프트 `"...준비되면 다음 지시를 대기하라"` 가 lead.md ⓐ 자동 착수 트리거와 정면 충돌. A 가 자동 착수한 건 ⓐ 가 이긴 1회 운, D 는 진 1회. **lead.md 본문을 아무리 손봐도 boot 입력 프롬프트 문구가 결정 변수**. 별도 보강 필요(Task 15 안건).
