...
# fixture 는 실 awa-up 띄우지 않고 tmux new-session + 직접 split 으로 구성.
set -uo pipefail
cd "$(dirname "$0")"
source ./assert.sh
ROOT="$(cd .. && pwd)"
...
TMP="$(mktemp -d)"; SAFE="$(basename "$TMP" | sed 's/[^A-Za-z0-9_-]/_/g')"; S="awa-$SAFE"
...
expected_format=' [ #{@awa-project-name} ] #{pane_title} '
...
echo "[W2] awa-up 으로 띄운 세션의 윈도우 구조"
...
# W2d: @awa-project-name = basename
got_pname="$(tmux show-options -v -t "$S2" @awa-project-name 2>/dev/null || echo MISS)"
assert_eq "$(basename "$TMP2")" "$got_pname" "W2d @awa-project-name = basename"
