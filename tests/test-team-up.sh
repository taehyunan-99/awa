...
# 1) 정상 생성
bash "$ROOT/bin/agenphony-up.sh" default
rc=$?
assert_eq "0" "$rc" "agenphony-up default 성공 종료"
...
# 3) 없는 프로파일 → 실패
bash "$ROOT/bin/agenphony-up.sh" nonexistent_profile
...
err_br="$(unset SESSION_OVERRIDE; HARNESS_PROJECT="$TMP_BR" AGENT_CMD=cat bash "$ROOT/bin/agenphony-up.sh" "$_pf_br" 2>&1 >/dev/null)"; rc_br=$?
_safe_br="$(printf '%s' "$(basename "$TMP_BR")" | sed 's/[^A-Za-z0-9_-]/_/g')"
tmux kill-session -t "agenphony-$_safe_br" 2>/dev/null || true
