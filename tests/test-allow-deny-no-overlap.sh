...
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/normal.yaml"
bash "$HARNESS_BIN/danger-check.sh" --check-allow-yaml "$TMPDIR/normal.yaml"
assert_success "$?" "정상 yaml PASS"

# 2. 위험 패턴 주입 — 충돌 발견 FAIL 기대
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/conflict.yaml"
...
malicious:
  ...
  - "Bash(rm:*)"
EOF
bash "$HARNESS_BIN/danger-check.sh" --check-allow-yaml "$TMPDIR/conflict.yaml" 2>/dev/null
rc=$?
assert_eq "1" "$rc" "위험 패턴 주입 yaml → 충돌 발견 (exit 1)"

# 3. yaml 미존재 — abort exit 1
bash "$HARNESS_BIN/danger-check.sh" --check-allow-yaml "$TMPDIR/notexist.yaml" 2>/dev/null
rc=$?
...
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/config/orch-auto-allow.yaml"
cp "$HARNESS_CONFIG/orch-auto-allow-stats.yaml" "$TMPDIR/config/orch-auto-allow-stats.yaml"
...
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/c2/config/orch-auto-allow.yaml"
cp "$HARNESS_CONFIG/orch-auto-allow-stats.yaml" "$TMPDIR/c2/config/orch-auto-allow-stats.yaml"
...
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/i7-conflict.yaml"
...
malicious-edit:
  ...
  - "Write(/etc/passwd)"
EOF
bash "$HARNESS_BIN/danger-check.sh" --check-allow-yaml "$TMPDIR/i7-conflict.yaml" 2>/dev/null
rc=$?
...
cp "$HARNESS_CONFIG/orch-auto-allow.yaml" "$TMPDIR/i8/config/orch-auto-allow.yaml"
cp "$HARNESS_CONFIG/orch-auto-allow-stats.yaml" "$TMPDIR/i8/config/orch-auto-allow-stats.yaml"
