...
- **v3** (방향성 리뷰): 사용자 원래 구상과 정합화. (A) 약한 신호 = scope 차단만, 의미 판정은 강한 신호(done 후)로 일원화 — 미완성 코드의 의미 판단은 거짓 양성/음성 불가피하므로. (B) 메인 = 단순 분배자 아닌 **프로젝트 관리자**(책임 9개: 분담·완료수신·리뷰종합·개입·산출물연결·보고·진도추적·품질게이트·전체맥락유지), 단계 전이는 사용자 통제, 맥락은 `.harness-state` 파일 보존. (C) codex=워커·리뷰어 둘 다 목표, `/loop`·hook 대체는 명시적 후속 과제(닫지 않고 열어둠). (E·F) 디바운스 claude 무상태 정합·done 라인 주체 명확화.
...
4. **codex 연동 확장점** — codex를 워커·리뷰어 둘 다로 쓰는 것이 목표. 본격 구현은 후속이며 `/loop`·hook 대체 메커니즘은 후속 과제(§7.2). 이번엔 워커 계약 명문화+분기점 격리로 막지 않음
...
**codex는 워커·리뷰어 둘 다 목표 — `/loop`·hook 대체는 후속 과제 (C)**: 위 계약 4가지는 워커용이다. 리뷰어 계약은 추가로 `/loop`(dynamic)+`Monitor` 자율 무장(상주 감시, §3.3)과 PostToolUse hook(events.log 결정적 기록, §5.6)에 의존하는데, 이 둘은 Claude Code 네이티브 기능이라 codex엔 동등 기능이 없다. **codex를 워커로도 리뷰어로도 쓰는 것이 목표**이되, codex 연동 시 이 두 기능을 대체할 메커니즘을 별도 설계해야 한다:

- `/loop` 상주 대체 후보: 외부 셸 watcher가 events.log 변경 시 codex 프로세스를 깨우기 (1차 토대 `wait-for` 메커니즘 재활용 가능)
- hook 결정적 기록 대체 후보: codex 측 도구 실행 후처리 훅, 또는 codex 산출물 diff로 변경 파일 역추적

이번 범위: codex 본격 구현 **안 함**(YAGNI). 단 (1) 워커 계약 4가지 명문화, (2) `team-up.sh`의 엔진 분기점을 `AGENT_CMD` 파싱 한 곳으로 격리 — codex 추가가 국소 변경이 되도록 보장. **codex의 `/loop`·hook 대체 설계는 명시적 후속 과제**(범위 밖이나 닫지 않고 열어둠 — codex 워커/리뷰어 모두 가능 목표).
...
- codex 본격 구현 (이번엔 워커 계약 명문화 + 분기점 격리만). **codex 워커·리뷰어의 `/loop`·hook 대체 메커니즘은 명시적 후속 과제** — 범위 밖이나 닫지 않음(§7.2)
