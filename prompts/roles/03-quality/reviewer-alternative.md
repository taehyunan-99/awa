...
`done` 라인 후 `.agent-harness/results/<id>.md`·산출물을 읽어 더 나은 대안이 있으면 `.agent-harness/review/<worker>-<id>.alternative-rev.md` 에 **verdict=SUGGESTION** 으로 기록한다(VIOLATION·blocking 아님). 대안이 없으면 verdict=OK. 진행을 막지 않는다 — lead 가 참고만 한다.
