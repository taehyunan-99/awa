...
## 강한 신호 (done 후)
`done` 라인(탭 5필드의 4번째 필드가 `done`) 후 `workspace/results/<id>.md`·산출물·관련 설계 문서를 읽어 아키텍처 일관성 위배를 판정한다. 위배 시 verdict=VIOLATION, signal=strong, OK 면 verdict=OK 를 같은 경로에 기록.
