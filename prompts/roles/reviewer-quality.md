...
## 강한 신호 (done 후)
`done` 라인(탭 5필드의 4번째 필드가 `done`) 후 `workspace/results/<id>.md`·산출물을 읽어 안티패턴·품질 문제(예: JWT 검증을 평문 비교)를 판정한다. 위배 시 verdict=VIOLATION, signal=strong, OK 면 verdict=OK 를 같은 경로에 기록.
