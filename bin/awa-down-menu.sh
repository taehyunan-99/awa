...
live=$(tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^awa-' || true)
...
case "$N" in
  ...
  1)
    ...
    [ "$ans" = "y" ] || exit 0
    [ "$proj" = "?" ] && { echo "오류: @awa-project 옵션 없음 ($s)" >&2; exit 1; }
    bash "$_DIR/awa-down.sh" --project "$proj"
    ;;
  *)
    i=0
    declare -a names
    ...
    for s in $selected; do
      proj=$(tmux show-options -t "$s" -v @awa-project 2>/dev/null || echo "")
      [ -n "$proj" ] && bash "$_DIR/awa-down.sh" --project "$proj"
    done
    ;;
esac
