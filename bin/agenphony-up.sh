...
if tmux has-session -t "$SESSION" 2>/dev/null; then
  echo "오류: 세션 '$SESSION' 이미 존재 (PROJECT_ROOT=$PROJECT_ROOT). attach 하거나 agenphony-down.sh 후 재시도." >&2
  exit 1
fi
...
if [ -f "$HARNESS_ROOT/templates/settings.json.tpl" ]; then
  ...
  printf 'generated by agent-harness — do not edit settings.json (regenerated by bin/agenphony-up.sh)\n' > "$MARKER"
fi
...
#   파일이 없으면 lookup 이 영구 rc=1 → lead-auto-allow 전체 무동작. 부트 시 설치.
...
HARNESS_YAML="$HARNESS_ROOT/config/lead-auto-allow.yaml"
PROJ_YAML="$PROJECT_ROOT/config/lead-auto-allow.yaml"
YAML_MARKER="$PROJECT_ROOT/config/.lead-auto-allow-marker"
if [ -f "$HARNESS_YAML" ]; then
  mkdir -p "$PROJECT_ROOT/config"
  if [ ! -f "$PROJ_YAML" ]; then
    # 최초 설치.
    cp "$HARNESS_YAML" "$PROJ_YAML"
    printf 'generated by agent-harness — do not edit (regenerated by bin/agenphony-up.sh)\n' > "$YAML_MARKER"
...
# probe 는 세션명 인자였으나 agenphony-up 은 pane_id/target 을 쓴다 — capture-pane/
...
# SKIPPED_PANES 누적 → T5 (agenphony-up 끝부분 가시화) 가 소비. 본 함수 단독으론 변수만 누적.
