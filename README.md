...
동일한 plan(재고·주문 트랜잭션 API — 인증·동시성 재고차감·결제 멱등성·취소 복원, 불변식 5종)을 단일 에이전트와 AWA로 각각 실행한 자체 실측.

| 지표 | 단일 에이전트 | AWA |
|---|---|---|
| 권한 질문(사람 개입) | **58회** | **2회** (28배↓) |
| 토큰 사용량 (5h 게이지 기준) | 15% | 15~19% (동급) |
| 작업 완주 | 통과 | 9 task 완주 · 테스트 64개 통과 · 약 40분 |
| 자기보고가 놓친 결함 | — (잡을 회로 없음) | **2건 실차단** (리뷰 합의 게이트) |
