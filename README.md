...
flowchart TB
    ...
    ORCH -->|"dispatch"| W["워커 — 작업에 맞춰 동적 조립<br/>역할별 권한 격리"]
    W -->|"task 완료"| R["투표 리뷰어 — 합의 게이트<br/>plan 정합 · 품질 · 보안"]
...
| **ORCH** | plan을 task로 분해·배정·종합 — 워커 개입(차단·수정 지시)의 독점권 |
| **워커** | 실제 작업자 (dev·tester·researcher …) — 작업 경로만 쓰기 가능한 권한 격리 |
| **투표 리뷰어** | plan 정합 · 코드 품질 · 보안 — **각자 독립 컨텍스트**에서 차단 투표 (1명 단독 거부권 없음) |
| **review-mgr** | 메타 리뷰 — plan 대비 드리프트를 시계열로 추적 (투표권 없음) |
| **watcher** | LLM이 아닌 데몬 — 파일 신호를 폴링해 필요한 에이전트만 깨움 (idle 비용 0) |

워커의 "끝났다"는 그대로 믿지 않는다 — task가 끝날 때마다 리뷰어들이 투표하고, 만장일치 차단이면 사람 없이 수정시킨다. 모든 신호는 파일 IPC로 흐르며(화면 파싱 없음), 감시는 네 축으로 상시 작동한다:
