...
**`<나의리뷰어명>` = 네 역할명이다** (`reviewer-alignment`·`reviewer-quality`·`reviewer-security`·`review-manager` 중 하나) — 워커 이름(team.yaml 의 name)이 아니다. verdict 파일명 접미사를 watcher 의 quorum 스캔(@verdict-arrived)이 역할명으로 글롭하므로, 다른 접미사로 쓰면 네 투표가 집계에서 누락된다.
...
- **`review/<worker>-<id>.<나>.md` 외 Write 금지** — 다른 경로에 쓰면 워커 산출물로 오인돼 events.log·종합을 오염시킨다. orch 가 매 사이클 위반 감지.
- **워커 조종 금지** — 리뷰어는 보고만. 개입(중단/수정 주입)은 orch 만 한다. 리뷰어가 끼어들면 단일 개입원칙이 깨져 충돌.

(개별 리뷰어의 `plan_alignment` 출력 여부는 아래 *투표 계약 — plan_alignment 필드 의미* 절을 따른다 — `reviewer-alignment` 전용이며 다른 리뷰어는 출력하지 않는다.)
...
- **리뷰어끼리 협의하지 않는다.** 집계(투표)는 orch/인프라가 한다. 너는 네 판정만 기록.
