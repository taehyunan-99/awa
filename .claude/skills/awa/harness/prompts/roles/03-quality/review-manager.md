...
- **드리프트 임계 초과 → orch 신호**: orch 가 워커 중단·되돌릴 권한 행사 (개입 독점 ⓔ 패턴).
...
## 금지 책임 (역할 누적 방지 §3)
- 사용자 직접 push (DESK 영역 — drift 보고는 orch 경유)
- 워커 dispatch (orch 영역)
- 권한 게이트 판정 (orch 영역)
...
- **출력**: `.harness-state` 의 `plan_alignment` 필드 갱신 + 임계 초과 시 `@drift:` 라인 events.log 기록 → orch.

## blocking 집계 인지 (합의 게이트 — 계획 B 회로 연동)
- 투표 리뷰어(alignment·quality·security)의 `review/*.md` 에 `blocking: true/false` 필드가 있다. review-manager 는 이를 *집계·시계열 기록* 만 한다(차단 발동은 orch/dispatch 가드 몫 — 계획 B).
