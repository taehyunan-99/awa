...
- `_partials/orch-gate.md` — orch 전용 권한 게이트 partial
- `roles/01-orchestration/{orch.md,desk.md}`
- `roles/02-development/{dev.md,researcher.md}`
- `roles/03-quality/{reviewer-alignment.md,reviewer-common.md,reviewer-quality.md,reviewer-security.md,review-manager.md,tester.md}`
...
- **reviewer 가 `@plan-defect:` / `@gate:` / `@done:` 출력 금지** — 워커 전용 채널. reviewer 가 잘못 출력하면 watcher 라우팅이 worker 로 오해 → orch 가 reviewer 를 worker 로 종합. `_common.md` 신호 토큰 절의 *워커 전용* 명시를 reviewer 부트 프롬프트에도 재진입.
...
- **orch.md cap 100줄 제한** — LLM 컨텍스트 비용 + 신호 진화 여유. 한 신호당 평균 3~5줄 추가 (절 ⓐ~ⓘ), 7종 신호 + 부가 절 까지 잡아도 ~60줄 → cap 100 은 *향후 3종 추가* 여유. cap 초과 시 신호 절 압축 (예: ⓖ 3줄 형식) 또는 partial 분리.
