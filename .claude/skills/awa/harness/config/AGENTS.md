...
`orch-auto-allow.yaml`은 카테고리(read-only / git-readonly / ...) 단위로 자동 허용할 도구 패턴(`Tool(prefix:*)` 또는 `Tool(exact)`)을 선언한다. danger_check 통과 후에만 도달하므로 위험 패턴 포함 금지 — 다층 방어.

## 2. CONTENTS

- `orch-auto-allow.yaml` — 카테고리 + 패턴 목록 (read-only, git-readonly, learned 등). `learned:` 는 `confirm_allow_yaml accepted` 가 누적하는 영역.
- `orch-auto-allow-blocklist.yaml` — 사용자 unsubscribe (Phase C 우회). `patterns:` 단일 노드 + `- "패턴"`. `confirm_allow_yaml never` 가 누적.
- `orch-auto-allow-stats.yaml` — Phase A 학습 통계 카운터. `patterns.<패턴>.<필드>: <수>` 3단 트리. `bump_stats_counter` 가 confirm/accepted/rejected/never 4 필드 누적.

세 yaml 의 구조가 의도적으로 다르다:
- `orch-auto-allow`: 카테고리 다중 (read-only / git-readonly / safe-test / ... + learned)
- `blocklist`: `patterns:` 단일 노드 (카테고리 분류 불필요 — 모두 사용자 거부 의미)
- `stats`: 3단 트리 (`patterns.<패턴>.<필드>`) — 패턴별 다필드 카운터

파일 형식 = 계약 (orch-auto-allow / blocklist 공통):
...
- **위험 패턴 추가 금지** (`sudo`, `rm -rf`, `git push --force`, `dd of=`, `curl | sh` 등) — `danger-check.sh` 통과 조건 위반. 다층 방어 원칙 (orch-auto-allow 는 *danger 통과 후에만 도달* 하는 안전 계층).
- **작업 파괴 패턴 학습 금지** (`Bash(git checkout:*)` / `Bash(git reset:*)` / `Bash(git clean:*)`) — `danger_check` 가 *전체* 를 잡지 않고 *옵션 조합 (--hard / -f / -fdx)* 만 잡음. `git checkout -- file` 의 작업 파괴 같은 *데이터 손실 잠재 위험* 은 자동 허용 카탈로그 진입 금지 (`orch-auto-allow.yaml` 의 `git-write` 카테고리 주석 참조).
...
- **3 yaml 분리 이유** — 읽기/쓰기 권한·수명·필드 의미가 서로 다르다.
  - `orch-auto-allow.yaml`: *운영 데이터*. 사람·`confirm_allow_yaml` 양쪽이 쓰기. 영구 보존.
  - `orch-auto-allow-blocklist.yaml`: *사용자 명시 결정*. `confirm_allow_yaml never` + 사용자 직접만 쓰기. 영구 보존.
  - `orch-auto-allow-stats.yaml`: *멀티 워커 누적 카운터*. `bump_stats_counter` 전용 쓰기. Phase B/C 진입 후 초기화 가능.
  세 가지 의미를 하나의 yaml 에 섞으면 사용자 편집·자동 갱신·통계 누적 책임 충돌.
- **다층 방어 (danger-check + orch-auto-allow)** — `orch-auto-allow` 가 *danger 를 통과한 명령에만* 적용. 즉 orch-auto-allow 에 위험 패턴 추가해도 *이미 danger-check 가 차단해서 도달 안 함*. 그러나 두 곳 정합이 깨지면 *조용한 우회* 가능 — 두 곳을 항상 일치시키기 위한 검증이 `danger-check.sh --check-allow-yaml` 가드 (Task 7 신규).
