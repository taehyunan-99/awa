...
> **핵심 원칙 (2026-06-06)**: profiles/*.yaml 은 *내부 참고용 시드*일 뿐, **사용자에게 통째로 던지지 않는다.** AWA(Claude)가 작업을 읽고 역할 카탈로그에서 필요한 것만 골라 조립하고 그 근거를 설명한다. "프로파일 5개 중 하나 고르기"는 정적 템플릿과 다를 게 없으므로 금지.
